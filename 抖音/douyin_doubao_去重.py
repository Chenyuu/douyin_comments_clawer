...
def read_existing_comments():
    """
    读取已有文档中的评论信息（视频URL+用户ID+评论内容）
    """
    global existing_comments
    if mode == '1':
        folder = '.'
        for filename in os.listdir(folder):
            if filename.startswith(f'{region_keyword}{keyword}') and filename.endswith('.docx'):
                doc = Document(os.path.join(folder, filename))
                current_video_url = None
                current_user_id = None
                for para in doc.paragraphs:
                    if para.text.startswith('视频链接:'):
                        current_video_url = para.text[len('视频链接:'):]
                    elif para.text.startswith('抖音号:'):
                        current_user_id = para.text[len('抖音号:'):]
                    elif para.text.startswith('评论:'):
                        comment = para.text[len('评论:'):]
                        if current_video_url and current_user_id and comment:
                            key = f"{current_video_url}::{current_user_id}::{comment}"
                            existing_comments.add(key)
    elif mode == '2':
        folder = '.'
        for filename in os.listdir(folder):
            if filename.startswith(f'{formatted_time[:10]}') and filename.endswith('-关注评论.docx'):
                doc = Document(os.path.join(folder, filename))
                current_video_url = None
                current_user_id = None
                for para in doc.paragraphs:
            ...
            if response:
                ...
                if json_data is not None and 'comments' in json_data:
                    ...
                    for comment in comments:
                        ...
                        # 生成去重键（视频URL+用户ID+评论内容）
                        key = f"{url}::{douyin_id}::{comment_content}"
                        # 检查评论是否已经存在
