...
async def download_image_async(url, name):
    ...
    :param name: 保存图片的文件名
    """
    await asyncio.sleep(random.uniform(MIN_DELAY, MAX_DELAY))  # 随机延迟
...
def get_comments(url, document):
    ...
    :param document: 文档对象
    """
    global total_comment_count, comments_per_video, insert_image
    # 开始监听评论列表请求
    dp1 = dpBox.new_tab()
    ...
    dp1.headers = headers
    if proxy:
        dp1.proxies = {'http': proxy, 'https': proxy}
    # 访问视频链接
    dp1.get(url)
                ...
                if json_data is not None and 'comments' in json_data:
                    ...
                    if comments is not None:
                        for comment in comments:
                            if current_comment_count >= comments_per_video:
...
def process_video(url, document):
    ...
    loop.run_until_complete(download_image_async(video_image_url, '1.png'))
...
def get_video_links():
    ...
    获取视频链接并处理评论信息
    """
    global keyword, region_keyword, comments_per_video, insert_image
    document = Document()
